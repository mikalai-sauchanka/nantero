...
namespace Nantero.Global
{
    ...
    public abstract class DBObject : IDatabaseObject
    {
        ...
        public void ForceId(ObjectId id)
        {
            Id = id;
        }
    ...
    {
        //private static readonly IProflowLogger _log = ProflowLogManager.GetLogger(false);
        private static readonly Object _syncRoot = new object();

        private readonly MongoDatabase _provider;
            ...
            if (_server == null)
            {
                lock (_syncRoot)
                {
                    if (_server == null)
                    {
                        ...
                        //   "Pactas Sloppy Conventions",
                        //   pack,
                        //   t => t.FullName.StartsWith("Pactas.Core."));
                    }
                }
            }
        ...
        public MongoContext(string databaseName)
        {
            ...
            _provider = CreateSession(databaseName);
            DatabaseName = databaseName;
        }

        public MongoContext(MongoContext other)
        {
            CodeContracts.Contract.Ensures(_provider != null);
            _provider = CreateSession(other.DatabaseName);
        ...
        public T FindModifyUpsert<T>(IMongoQuery query, IMongoUpdate update, WriteConcern writeConcern)
        {
            ...
            var result = GetCollection<T>(writeConcern).FindAndModify(query, SortBy.Null, update, true, true);
        ...
        public T FindModifyUpsert<T>(IMongoQuery query, IMongoUpdate update)
        {
            ...
            var result = GetCollection<T>().FindAndModify(query, SortBy.Null, update, true, true);
            return result.GetModifiedDocumentAs<T>();
        }
        ...
        private T FindAndModifyBase<T>(IMongoQuery query, IMongoUpdate update, bool returnNew)
        {
            try
            {
                var temp = GetCollection<T>().FindAndModify(query, SortBy.Null, update, returnNew);
